...
function StatisticsProd() {
    ...
    const handleDateChange = (url, page = 1, resetPage = false) => {
        ...
        apiCall(url, 'POST', requestData, null)
            .then((response) => {
                //alert(`** 서버 응답 Prod: ${JSON.stringify(response)}`);
                //prompt("복사하려면 Ctrl+C:", `${JSON.stringify(response)}`);
                ...
                if ( response === undefined || response === null || response.length === 0 ) {
                    ...
                    const formattedChartData = response.map(item => ({
                        name: item.prod_nm,
                        count: (item.sum_cnt ?? 0).toLocaleString('ko-KR'),
                    }));
                    ...
                    //     prodSumCnt: item.SUM_CNT,
                    //     prodSumBuyPrice: item.SUM_BUY_PRICE,
                    //     count: item.SUM_CNT,
                    //     prodCnt: item.PROD_CNT,
                    //     rownum: item.ROWNUM,
                    // }));

                    const formattedData = response.map(item => ({
                        prodNm: item.prod_nm,
                        prodPrice: (item.prod_price ?? 0).toLocaleString('ko-KR'),
                        prodSumCnt: (item.sum_cnt ?? 0).toLocaleString('ko-KR'),
                        prodSumBuyPrice: (item.sum_buy_price ?? 0).toLocaleString('ko-KR'),
                        count: (item.sum_cnt ?? 0).toLocaleString('ko-KR'),
                        prodCnt: (item.prod_cnt ?? 0).toLocaleString('ko-KR'),
                        rownum: item.rownum,
                    }));
